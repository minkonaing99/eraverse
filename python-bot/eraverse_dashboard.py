...
def fetch_products():
    """Fetch all products from the unified products_catalog table"""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
        cursor.execute("SELECT product_id, product_name, duration, wholesale, retail FROM products_catalog ORDER BY product_name")
...
def fetch_product_details(product_id):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM products_catalog WHERE product_id = %s", (product_id,))
...
def get_summary_data(date_str):
    ...
    try:
        ...
        # Get sales and profit for the date
        cursor.execute("""
...
def get_today_sales_details():
    ...
    try:
        ...
        cursor.execute("""
            SELECT sale_product, customer, price, profit, manager
            FROM sale_overview
            WHERE purchased_date = %s
...
def get_expiring_soon_products():
    ...
    try:
        ...
        cursor = conn.cursor(dictionary=True)
        cursor.execute("""
            SELECT sale_product, customer, email, purchased_date, expired_date
            FROM sale_overview
...
def save_sale(data):
    """Save a new sale to the sale_overview table"""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor()
...
def get_renewals_due_soon():
    """Get subscriptions that need renewal within 3 days"""
    try:
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor(dictionary=True)
        
        # Get all sales with renew > 0
        cursor.execute("""
            SELECT sale_product, customer, email, purchased_date, expired_date, duration, renew
            FROM sale_overview
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("Add New Sale", callback_data='add_sale')],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'add_sale':
        ...
        keyboard, row = [], []
        for i, (product_id, name, duration, wholesale, retail) in enumerate(products, 1):
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(lines) >= 3:
        ...
        price = float(lines[3]) if len(lines) >= 4 and lines[3].replace('.', '').isdigit() else float(product['retail'])
        
        # Calculate profit
        profit = price - float(product['wholesale'])
        ...
        await update.message.reply_text("Invalid input format. Please provide at least: Customer, Email, Manager\n\nPlease try again with the correct format:")
        
        # Don't clear context, let user try again
        return
    ...
    if today_sales_details:
        ...
        for i in range(0, len(today_sales_details), batch_size):
            ...
            for j, sale in enumerate(batch, i + 1):
                ...
                batch_response += f"{sale['customer']}\n"
                batch_response += f"{int(sale['price'])} Ks\n\n"
